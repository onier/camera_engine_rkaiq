...
XCamReturn
FakeCamHwIsp20::setIspParams(SmartPtr<RkAiqIspParamsProxy>& ispParams)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
...
XCamReturn
FakeCamHwIsp20::hdr_mipi_prepare(int idx)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (ret < 0)
            LOGE_CAMHW_SUBM(FAKECAM_SUBM, "mipi tx:%d prepare err: %d\n", ret);
        ret = _mipi_rx_devs[i]->prepare();
        if (ret < 0)
            LOGE_CAMHW_SUBM(FAKECAM_SUBM, "mipi rx:%d prepare err: %d\n", ret);
    }
...
XCamReturn
FakeCamHwIsp20::hdr_mipi_prepare_mode(int mode)
{
    XCamReturn ret = XCAM_RETURN_NO_ERROR;
    ...
    if (!mNormalNoReadBack) {
        ...
        if (ret < 0) {
            LOGE_CAMHW_SUBM(FAKECAM_SUBM, "hdr mipi start err: %d\n", ret);
        }
    }

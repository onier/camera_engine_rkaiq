...
Isp20SpThread::select_motion_params(RKAnr_Mt_Params_Select_t *stmtParamsSelected, uint32_t frameid)
{
    ...
    float gain                              = iso / 50;
    float gain_f                            = log(gain) / log(2);
    uint8_t gain_l                          = ceil(gain_f);
    uint8_t gain_r                          = floor(gain_f);
    float ratio                             = (float)(gain_l - gain_f);
...
int get_wr_flg_func()
{
    int write_flg = WRITE_FLG;
    if(access("/tmp/motion_detection_wr_flg",F_OK)==0)
        write_flg = 1;
    return write_flg;
}
...
Isp20SpThread::start()
{
    ...
    if (create_stop_fds_ispsp()) {
        LOGE_CAMHW_SUBM(ISP20POLL_SUBM, "create ispsp stop fds failed !");
        return;
    }
    mKgThread->start();
    mWrThread->start();
    ...
    LOGD("start tnr process,ret=%d",ret);
}
...
Isp20SpThread::stop()
{
    ...
    tnr_trigger.on = 0;
    int ret = _ispp_dev->io_control(RKISPP_CMD_TRIGGER_MODE, &tnr_trigger);
    LOGD("stop tnr process,ret=%d",ret);
...
Isp20SpThread::subscrible_ispgain_event(bool on) {
  ...
  if (ret) {
      LOGE("can't subscribe TNR complete event!\n");
  }
  return 0;
}
...
bool Isp20SpThread::init_fbcbuf_fd()
{
    ...
    if (res)
        return false;
    LOGD("ispbuf_num=%d",ispbuf_fd.buf_num);
...
bool Isp20SpThread::init_tnrbuf_fd()
{
    ...
    if (res)
        return false;
    LOGD("isppbuf_num=%d",isppbuf_fd.buf_num);
...
Isp20SpThread::kg_proc_loop ()
{
    ...
    if (poll_ret == POLL_STOP_RET) {
        LOGW("poll ispp stop sucess !");
        // stop success, return error to stop the poll thread
        return XCAM_RETURN_ERROR_UNKNOWN;
    }
    if (poll_ret <= 0) {
        LOGW( "ispp poll buffer event got error(%d) but continue\n", poll_ret);
    ...
    if(frame_detect_flg[static_ratio_idx_out])
    {
        ...
        if (MAP_FAILED == gainkg_addr) {
            LOGE("mmap gainkg_fd failed!!!(errno=%d),fd: %d, idx:%u, size: %d", errno, kg_fd, tnr_inf->gainkg_idx, tnr_inf->gainkg_size);
            LOGE("\n*** ASSERT: In File %s,line %d ***\n", __FILE__, __LINE__);
            assert(0);
        }

        gettimeofday(&tv1, NULL);
        set_gainkg(gainkg_addr,     ratio);
        ...
        LOG1("set_gain_kg  fid %u %8ld %8ld %8ld %8ld %8ld delta %8ld %8ld %8ld %8ld \n",tnr_inf->frame_id,
...
Isp20SpThread::wr_proc_loop ()
{
    ...
    while (loop_live) {
        ...
        switch(msg->cmd)
        {
            ...
            {
                LOG1("MSG_CMD_WR_START received");
                ratio_idx = msg->arg1;
                ...
                if (MAP_FAILED == gainwr_addr) {
                    LOGE("mmap gainwr_fd failed!!!(errno=%d),fd: %d, size: %d", errno, wr_fd, tnr_info->gainwr_size);
                    LOGE("\n*** ASSERT: In File %s,line %d ***\n", __FILE__, __LINE__);
                    assert(0);
                }
        	    ...
        	    LOG1("set_gain_wr fid %u %8ld %8ld %8ld %8ld %8ld delta %8ld %8ld %8ld %8ld \n", tnr_info->frame_id,
        	        tv0.tv_usec, tv1.tv_usec, tv2.tv_usec, tv3.tv_usec, tv4.tv_usec, tv1.tv_usec - tv0.tv_usec,
        	        tv2.tv_usec - tv1.tv_usec, tv3.tv_usec - tv2.tv_usec, tv4.tv_usec - tv3.tv_usec  );
                break;
            }
...
Isp20SpThread::loop () {
    ...
    if (_fd_init_flag) {
        ...
        LOGD( "isp&ispp buf fd init success!");
        _fd_init_flag = false;
    }

    poll_ret = _isp_sp_dev->poll_event(1000, ispsp_stop_fds[0]);
    if (poll_ret == POLL_STOP_RET) {
        LOGW("poll isp sp stop sucess !");
        // stop success, return error to stop the poll thread
        return false;
    }

    if (poll_ret <= 0) {
        LOGW( "isp sp poll buffer event got error(%d) but continue\n", poll_ret);
        return true;
    }

    ret = _isp_sp_dev->dequeue_buffer (buf);
    if (ret != XCAM_RETURN_NO_ERROR) {
        LOGE_CAMHW_SUBM(ISP20POLL_SUBM, "dequeue isp_sp_dev buffer failed");
        return false;
    }
    ...
    LOG1("loop frame_num_isp %d fid %u\n", frame_num_isp, ispgain->frame_id);
    ...
    LOG1("loop frame_num_isp %d fid %u flg %d %d\n", frame_num_isp, ispgain->frame_id, detect_flg, detect_flg_last);
    ...
    if(detect_flg)
    {
        if(frame_write_st == -1 && get_wr_flg_func())
            frame_write_st = MAX(write_frame_num_st, frame_num_isp + 1);
        int wr_flg = get_wr_flg_func() && (frame_num_isp >= frame_write_st && frame_num_isp < frame_write_st + write_frame_num);
        ...
        if(1)
        {
            ...
            LOG1("loop gain_dmaidx %u gain_fd %d mfbc_dmaidx %u mfbc_fd %d\n", ispgain->gain_dmaidx, gain_fd, ispgain->mfbc_dmaidx, mfbc_fd);
            void *gain_addr                             = mmap(NULL, ispgain->gain_size, PROT_READ | PROT_WRITE, MAP_SHARED, gain_fd, 0);
            if (MAP_FAILED == gain_addr) {
                LOGE("mmap gain_dmafd failed!!!(errno:%d),fd: %d, idx: %u, size: %d", errno, gain_fd, ispgain->gain_dmaidx, ispgain->gain_size);
                LOGE("\n*** ASSERT: In File %s,line %d ***\n", __FILE__, __LINE__);
                assert(0);
            }
            ...
            if (MAP_FAILED == mfbc_addr) {
                LOGE("mmap mfbc_dmafd failed!!!(errno:%d),fd: %d, idx: %u, size: %d", errno, mfbc_fd, ispgain->mfbc_dmaidx, ispgain->mfbc_size);
                LOGE("\n*** ASSERT: In File %s,line %d ***\n", __FILE__, __LINE__);
                assert(0);
            }
            ...
            if(wr_flg && WRITE_FLG_OTHER)
            {
                static    FILE *fd_ds_wr    = NULL;
                if(fd_ds_wr == NULL)
                    fd_ds_wr                        = fopen("/tmp/ds_out_last.yuv", "wb");
                if(fd_ds_wr)
                    fwrite(pPreIn, img_buf_size + img_buf_size_uv,        1,   fd_ds_wr);
            }
            if(wr_flg && WRITE_FLG_OTHER)
            {
	        ...
	        if(detect_flg_last == 1)
            {
                ...
                motion_detect(pCurIn, pPreIn, pTmpBuf, static_ratio_cur, pPreAlpha, (uint8_t*)src, _img_height_align, _img_width_align, _img_height, _img_width,
                              gain_blk_isp_stride, mtParamsSelect.sigmaHScale, mtParamsSelect.sigmaLScale, mtParamsSelect.uv_weight, mtParamsSelect.light_clp);
                gettimeofday(&tv3, NULL);
                LOG1("set_gain_isp frame_write_st %d time %8ld %8ld %8ld %8ld delta %8ld %8ld %8ld %8ld %d %x\n",frame_write_st,
                ...
                #if 0
                for(int i = 0; i < gain_kg_tile_h_align; i++)
                    for(int j = 0; j < ratio_stride; j++)
                    {
                        int idx_ratio                           = i * ratio_stride + j;
                        static_ratio_cur[idx_ratio]             = ROUND_INT((static_ratio_l - static_ratio_cur[idx_ratio])*(static_ratio_l - static_ratio_cur[idx_ratio]) * static_ratio_delta, (static_ratio_bit_inte + static_ratio_bit_deci)*2);
                        static_ratio_cur[idx_ratio]             = CLIP(static_ratio_cur[idx_ratio], 0, static_ratio_delta);
                        static_ratio_cur[idx_ratio]             = static_ratio_cur[idx_ratio] + static_ratio_r;
                    }
                if(MOTION_DETECTION_FLG1)
                    memset(static_ratio_cur, static_ratio_l, gain_kg_tile_h_align * ratio_stride);
                #if 0
                uint8_t *tmp  = (uint8_t*)malloc(gain_blk_isp_h * gain_blk_isp_stride * sizeof(tmp[0]));
                uint8_t *gain_table = (uint8_t*)gain_addr;
                for(int i = 0; i < gain_blk_isp_h; i++)
                    for(int j = 0; j < gain_blk_isp_stride; j++)
                    {
                        int idx                     = i * gain_blk_isp_stride + j;
                        int idx_ratio               = i * ratio_stride + j;
                        gain_table[idx]             = ROUND_INT((uint16_t)gain_table[idx] * static_ratio_cur[idx_ratio], static_ratio_l_bit) ;
                    }
                #else
                set_gain_isp(gain_addr, static_ratio_cur);
                #endif
                #endif
            }
            else
    ...
    if(_first_frame) {
        ...
        _buf_list_mutex.lock();
        LOG1("v4l2buf index %d push list\n", buf_proxy->get_v4l2_buf_index());
        _isp_buf_list.push_back(buf_proxy);
        _buf_list_mutex.unlock();
    }

    frameid_ptr[static_ratio_idx_in]        = ispgain->frame_id;
...
Isp20SpThread::set_sp_img_size(int w, int h, int w_align, int h_align)
{
    ...
    LOGD("_img_height %d %d _img_width %d %d\n", h, h_align, w, w_align);
...
Isp20SpThread::set_gain_isp(void *buf, uint8_t* ratio)
{
    ...
    uint8_t *gain_table = (uint8_t*)buf;
    for(int i = 0; i < gain_blk_isp_h; i++)
        for(int j = 0; j < gain_blk_isp_stride; j++)
        {
            int idx                     = i * gain_blk_isp_stride + j;
            int idx_ratio               = i * ratio_stride + j;
            gain_table[idx]             = ROUND_INT((uint16_t)gain_table[idx] * ratio[idx_ratio], static_ratio_l_bit) ;
        }
}
...
Isp20SpThread::set_gain_wr(void *buf, uint8_t* ratio)
{
    ...
    int wr_flg                          = get_wr_flg_func() && (frame_num_pp >= frame_write_st && frame_num_pp < frame_write_st + write_frame_num);
    LOG1("set_gain_wr frame_num_pp %d frame_write_st %d  write_frame_num %d\n ",frame_num_pp, frame_write_st, write_frame_num);
    wr_flg &= WRITE_FLG_OTHER;
  //  wr_flg = 0;
    uint8_t *test_buff[2];
    uint8_t *test_buff_ori[2];
    uint8_t *gain_isp_buf_ds;
    if(wr_flg)
    {
        for(int i = 0; i < 2; i++)
        {
        ...
        if(0)
        {
            static FILE *fd = NULL;
            if(fd == NULL)
                fd = fopen("/tmp/gain_pp_ratio_out.yuv", "wb");
            if(fd)
                fwrite(src, ratio_stride * gain_kg_tile_h_align, 1, fd);
        }
    }

    uint8_t *gain_isp_buf_cur               = gain_isp_buf_bak[static_ratio_idx_out];

    LOG1("mtParamsSelect.yuvnr_gain_scale %f %f %f\n", mtParamsSelect.yuvnr_gain_scale[0], mtParamsSelect.yuvnr_gain_scale[1], mtParamsSelect.yuvnr_gain_scale[2]);
#ifndef ENABLE_NEON
    int flg = 0;
...
void
Isp20SpThread::set_gainkg(void *buf, uint8_t* ratio)
{
    ...
    LOG1("set_gain_kg frame_num_pp %d frame_write_st %d  write_frame_num %d\n ",frame_num_pp, frame_write_st, write_frame_num);
   ...
   // wr_flg = 0;
    ...
    if(wr_flg)
    {
        ...
        if(fd_gainkg_out)
            fwrite(src, gain_tile_gainkg_stride * gain_tile_gainkg_h,  1, fd_gainkg_out);
        static FILE *fd = NULL;
        if(fd == NULL)
            fd = fopen("/tmp/gainkg_ratio_out.yuv", "wb");
        if(fd)
            fwrite(ratio, ratio_stride * gain_kg_tile_h_align,  1, fd);
    }
    int frame_limit_div_y   = 256 / sqrt(mtParamsSelect.frame_limit_y);
    int frame_limit_div_uv  = 256 / sqrt(mtParamsSelect.frame_limit_uv);
 //   printf("frame_limit_div_y %d %d %d %d\n",frame_limit_div_y, frame_limit_div_uv);
             ...
             //   for(int tile_idx = 0; tile_idx < gainkg_tile_num; tile_idx++)
                {
                    ...
                    for(int y = 0; y < gain_tile_ispp_y; y++)
					{
                        ...
                        for(int x = 0; x < gain_tile_ispp_x; x++)
                        {
                            ...
                            src[idx_gain + 0]           = MIN(255,(src[idx_gain + 0]    << static_ratio_l_bit) / ratio_cur);
                            src[idx_gain + 2]           = MIN(255,(src[idx_gain + 2]    << static_ratio_l_bit) / ratio_cur);
                            src[idx_gain + 4]           = MIN(255,(src[idx_gain + 4]    << static_ratio_l_bit) / ratio_cur);
                            src[idx_gain + 6]           = MIN(255,(src[idx_gain + 6]    << static_ratio_l_bit) / ratio_cur);
...
Isp20SpThread::init()
{
    ...
    _fd_init_flag = true;
    _first_frame = true;
    _motion_params = _calibDb->mfnr.mode_cell[0].motion;
    ...
    static_ratio_bit_inte               = RATIO_BITS_NUM - RATIO_BITS_R_NUM;
    static_ratio_bit_deci               = static_ratio_l_bit - static_ratio_bit_inte;
    static_ratio_max                    = 1 << static_ratio_bit_inte;
    static_ratio_l                      = 1 << static_ratio_l_bit;
    static_ratio_r                      = 1 << (static_ratio_bit_deci);
    static_ratio_delta                  = static_ratio_l - static_ratio_r;
    ...
    for(int i = 0; i < static_ratio_num; i++)
    {
        ...
        for(int i = gain_blk_isp_h; i < gain_kg_tile_h_align; i++)
            for(int j = gain_blk_isp_w; j < ratio_stride; j++)
            {
                int idx                 = i * gain_blk_isp_stride + j;
                static_ratio[i][idx]    = static_ratio_r ;
            }
    }
    ...
    frameid_ptr                         = (int*)malloc(static_ratio_num * sizeof(frameid_ptr[0]));
    frameid_tnr_ptr                     = (int*)malloc(static_ratio_num * sizeof(frameid_tnr_ptr[0]));

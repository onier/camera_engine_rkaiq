...
static int height = 1538;
static int type  = 0;
rk_aiq_working_mode_t  mode = RK_AIQ_WORKING_MODE_ISP_HDR3;
static const char* sens_entity_name = NULL;
...
{
    aiq_ctx = rk_aiq_uapi_sysctl_init(sens_entity_name, NULL, NULL, NULL);
    DBG("rkaiq  prepare width = %d, height = %d, mode = %d\n", width , height, mode);
...
void parse_args(int argc, char **argv)
{
   ...
   while (1) {
       ...
       c = getopt_long(argc, argv, "m:w:h:t:s:h", long_options, &option_index);
       if (c == -1)
           break;
       ...
       case 's':
           ...
           break;
       case '?':
           ERR("Usage: %s -m /dev/media0 -w 2688 -h 1520 -t 0\n"
               "         --mmedia,  optional, mapped media device node\n"
               "         --width,   optional, sensor prepare width\n"
               "         --height,  optional, sensor prepare height\n"
               "         --type,    optional, rkaiq working mode: 0->normal 1->HDR2 2->HDR3\n"
   ...
   if (!mdev_path) {
		...
		"Usage: %s -m /dev/media0 -w 2688 -h 1520 -t 0\n", argv[0]);
        exit(-1);
	}
}
